...
```{r DC-four-2-check}
test_0 <- fcall("data(CPS85, whatever)", "the first argument to `data()` is the data table, e.g. `CPS85`")
test_1 <- fcall("data(whatever, whatever)", "the second argument to `data()` is `package =`")
test_2 <- fcall("head(CPS85)", "give the data table as the argument to `head()`")
test_3 <- check_argument("data(CPS85, package = grab_this)", 
                         test = function(x) 
                           if(x == 'mosaicData') return("") else return("should be the name of the package (in quotes)."))
# test_3 isn't working for some reason. Try another
test_3 <- fcall("data(whatever, package = 'mosaicData')", message = "argument should be the name of the package (in quotes).")
...
test_1 <- fcall("read.csv(whatever)")
test_2 <- in_statements("[\"'].*[\"']", 
                        message = "you don't have the file name in quotes.",
                        regex = TRUE)
test_3 <- in_statements("[\"']http://tiny.cc/dcf/CountryCodes.csv[\"']", regex = TRUE,
                        "you don't have the name of the file typed correctly.")
test_4 <- assigns_to("CCodes", message = "remember to assign the output to `CCodes`.")
